...
let ClinicsService = class ClinicsService {
    ...
    async completeClinic(clinicId, dto, file) {
        return this.dataSource.transaction(async (manager) => {
            ...
            const incomingMap = new Map(dto.workingHours.map(wh => [wh.dayOfWeek, wh]));
            const updatedHours = [];
            ...
            for (const incoming of incomingMap.values()) {
                const newHour = manager.create(clinic_working_hours_entity_1.ClinicWorkingHours, {
            ...
            const toRemove = existingHours.filter(h => !daysToKeep.includes(h.dayOfWeek));
            if (toRemove.length) {
                await manager.remove(clinic_working_hours_entity_1.ClinicWorkingHours, toRemove);
            }
